...
const Header: React.FC<HeaderProps> = ({ handleDrawerToggle }) => {
  ...
  const cartItemsCount = useSelector(
    (state: RootState) => state.cartItemsCount.value
  );
  const dispatch = useDispatch<AppDispatch>();
  const [field, setField] = useState<string>("all");
  const [keyword, setKeyword] = useState<string>("");
  const throttledKeyword = useThrottle(keyword, 500);
  const [searchResults, setSearchResults] = useState<Map<string, IProduct[]>>(
    new Map()
  );
  ...
  const searchProducts = useCallback(async () => {
    ...
    const searchRes = ProductService.getSearchResultMap(
      throttledKeyword,
      products
    );
  ...
  useEffect(() => {
    if (user) {
      dispatch({
        type: cartItemsCountActionTypes.SET,
        payload: cartItemsCountInit,
      });
    }
  ...
  return (
    <AppBar
      position="sticky"
      color="primary"
      sx={{ height: "100%", width: "100%", backgroundColor: "#111827" }}
    >
      <Toolbar
        sx={{
          justifyContent: "center",
